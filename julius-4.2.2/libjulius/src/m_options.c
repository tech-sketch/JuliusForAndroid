...
#define GET_TMPARG  if ((tmparg = next_arg(&i, argc, argv)) == NULL) return FALSE
  ...
  for (i=1;i<argc;i++) {
    ...
    if (strmatch(argv[i],"-C")) { /* include jconf file  */
...
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-C error");
#endif
        return FALSE;
      }
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-C:%s", tmparg);
#endif
      free(tmparg);
      continue;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-AM %s", tmparg);
#endif
      jconf->amnow = amconf;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-AM_GMM");
#endif
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-LM %s", tmparg);
#endif
      jconf->lmnow = lmconf;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-SR %s %s", tmparg[0], tmparg[1]);
#endif
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-GLOBAL");
#endif
      continue;
    } else if (strmatch(argv[i],"-sectioncheck")) { /* enable section check */
      jconf->optsectioning = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-sectioncheck");
#endif
      continue;
    } else if (strmatch(argv[i],"-nosectioncheck")) { /* disable section check */
      jconf->optsectioning = FALSE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-nosectioncheck");
#endif
      continue;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-input %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-filelist")) {	/* input file list */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-filelist check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-filelist %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-rejectshort")) { /* short input rejection */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-rejectshort check_section error");
#endif
        return FALSE; 
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-rejectshort %s", tmparg);
#endif
      continue;
#ifdef POWER_REJECT
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-powerthres check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-powerthres %s", tmparg);
#endif
      continue;
#endif
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-force_realtime check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-force_realtime:%s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-realtime")) {	/* equal to "-force_realtime on" */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-realtime check_section error");
#endif
        return FALSE;
      }
      jconf->decodeopt.forced_realtime = TRUE;
      jconf->decodeopt.force_realtime_flag = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-realtime");
#endif
      continue;
    } else if (strmatch(argv[i], "-norealtime")) { /* equal to "-force_realtime off" */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-norealtime check_section error");
#endif
        return FALSE;
      }
      jconf->decodeopt.forced_realtime = FALSE;
      jconf->decodeopt.force_realtime_flag = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-norealtime");
#endif
      continue;
    } else if (strmatch(argv[i],"-forcedict")) { /* skip dict error */
      if (!check_section(jconf, argv[i], JCONF_OPT_LM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-forcedict check_section error");
#endif
        return FALSE;
      }
      jconf->lmnow->forcedict_flag = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-forcedict");
#endif
      continue;
    } else if (strmatch(argv[i],"-check")) { /* interactive model check mode */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-check check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-check %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-notypecheck")) { /* don't check param type */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-notypecheck check_section error");
#endif
        return FALSE;
      }
      jconf->input.paramtype_check_flag = FALSE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-notypecheck");
#endif
      continue;
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-nlimit check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#endif
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-nlimit %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-lookuprange")) { /* trellis neighbor range */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-lookuprange check_section error");
#endif
        return FALSE; 
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-lookuprange %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-graphout")) { /* enable graph output */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-graphout check_section error");
#endif
        return FALSE;
      }
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-graphout");
#endif
      continue;
    } else if (strmatch(argv[i],"-lattice")) { /* enable graph output */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-lattice check_section error");
#endif
        return FALSE;
      }
      jconf->searchnow->graph.enabled = TRUE;
      jconf->searchnow->graph.lattice = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-lattice");
#endif
      continue;
    } else if (strmatch(argv[i],"-nolattice")) { /* disable graph output */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-nolattice check_section error");
#endif
        return FALSE;
      }
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-nolattice");
#endif
      continue;
    } else if (strmatch(argv[i],"-confnet")) { /* enable confusion network */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-confnet check_section error");
#endif
        return FALSE;
      }
      jconf->searchnow->graph.enabled = TRUE;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-confnet");
#endif
      continue;
    } else if (strmatch(argv[i],"-noconfnet")) { /* disable graph output */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-noconfnet check_section error");
#endif
        return FALSE;
      }
      jconf->searchnow->graph.enabled = FALSE;
      jconf->searchnow->graph.confnet = FALSE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-noconfnet");
#endif
      continue;
    } else if (strmatch(argv[i],"-graphrange")) { /* neighbor merge range frame */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-graphrange check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-graphrange %s", tmparg);
#endif
      continue;
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-graphcut check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-graphcut %s", tmparg);
#endif
      continue;
#endif
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-graphboundloop check_section error");
#endif
        return FALSE; 
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-graphboundloop %s", tmparg);
#endif
      continue;
#endif
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-graphsearchdelay check_section error");
#endif
        return FALSE;
      }
      jconf->searchnow->graph.graphout_search_delay = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-graphsearchdelay");
#endif
      continue;
    } else if (strmatch(argv[i],"-nographsearchdelay")) { /* not do graph search termination before the 1st sentence is found */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-nographsearchdelay check_section error");
#endif
        return FALSE;
      }
      jconf->searchnow->graph.graphout_search_delay = FALSE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-nographsearchdelay");
#endif
      continue;
#endif
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-looktrellis check_section error");
#endif
        return FALSE; 
      }
      jconf->searchnow->pass2.looktrellis_flag = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-looktrellis");
#endif
      continue;
    } else if (strmatch(argv[i],"-multigramout")) { /* enable per-grammar decoding on 2nd pass */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-multigramout check_section error");
#endif
        return FALSE;
      }
      jconf->searchnow->output.multigramout_flag = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-multigramout");
#endif
      continue;
    } else if (strmatch(argv[i],"-nomultigramout")) { /* disable per-grammar decoding on 2nd pass */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-nomultigramout check_section error");
#endif
        return FALSE;
      }
      jconf->searchnow->output.multigramout_flag = FALSE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-nomultigramout");
#endif
      continue;
    } else if (strmatch(argv[i],"-oldtree")) { /* use old tree function */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-oldtree check_section error");
#endif
        return FALSE;
      }
      jconf->searchnow->pass1.old_tree_function_flag = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-oldtree");
#endif
      continue;
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-sb check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-sb %s", tmparg);
#endif
#else
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-bs check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-bs %s", tmparg);
#endif
      continue;
#endif
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-cutsilence check_section error");
#endif
        return FALSE;
      }
      jconf->detect.silence_cut = 1;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-cutsilence");
#endif
      continue;
    } else if (strmatch(argv[i],"-nocutsilence")) { /* force (long) silence detection off */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-nocutsilence check_section error");
#endif
        return FALSE;
      }
      jconf->detect.silence_cut = 0;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-nocutsilence");
#endif
      continue;
    } else if (strmatch(argv[i],"-pausesegment")) { /* force (long) silence detection on (for backward compatibility) */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-pausesegment check_section error");
#endif
        return FALSE; 
      }
      jconf->detect.silence_cut = 1;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-pausesegment");
#endif
      continue;
    } else if (strmatch(argv[i],"-nopausesegment")) { /* force (long) silence detection off (for backward comatibility) */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-nopausesegment check_section error");
#endif
        return FALSE; 
      }
      jconf->detect.silence_cut = 0;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-nopausesegment");
#endif
      continue;
    } else if (strmatch(argv[i],"-lv")) { /* silence detection threshold level */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-lv check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-lv %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-zc")) { /* silence detection zero cross num */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-zc check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-zc %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-headmargin")) { /* head silence length */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-headmargin check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-headmargin %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-tailmargin")) { /* tail silence length */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-tailmargin check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-tailmargin %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-chunksize")) { /* chunk size for detection */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-chunksize check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-chunksize %s", tmparg);
#endif
      continue;
...
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-hipass check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-hipass %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-lopass")||strmatch(argv[i],"-lofreq")) { /* frequency of lower band limit */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-lopass check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-lopass %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-smpPeriod")) { /* sample period (ns) */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-smpPeriod check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-smpPeriod %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-smpFreq")) { /* sample frequency (Hz) */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-smpFreq check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-smpFreq %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-fsize")) { /* Window size */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-fsize check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-fsize %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-fshift")) { /* Frame shiht */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-fshift check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-fshift %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-preemph")) {
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-preemph check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-preemph %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-fbank")) {
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-fbank check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-fbank %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-ceplif")) {
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-ceplif check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-ceplif %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-rawe")) {
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-rawe check_section error");
#endif
        return FALSE;
      }
      jconf->amnow->analysis.para.raw_e = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-rawe");
#endif
      continue;
    } else if (strmatch(argv[i],"-norawe")) {
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-norawe check_section error");
#endif
        return FALSE;
      }
      jconf->amnow->analysis.para.raw_e = FALSE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-norawe");
#endif
      continue;
    } else if (strmatch(argv[i],"-enormal")) {
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-enormal check_section error");
#endif
        return FALSE;
      }
      jconf->amnow->analysis.para.enormal = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-enormal");
#endif
      continue;
    } else if (strmatch(argv[i],"-noenormal")) {
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-noenormal check_section error");
#endif
        return FALSE;
      }
      jconf->amnow->analysis.para.enormal = FALSE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-noenormal");
#endif
      continue;
    } else if (strmatch(argv[i],"-escale")) {
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-escale check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-escale %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-silfloor")) {
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-silfloor check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-silfloor %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-delwin")) { /* Delta window length */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-delwin check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-delwin %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-accwin")) { /* Acceleration window length */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-accwin check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-accwin %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-ssalpha")) { /* alpha coef. for SS */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-ssalpha check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-ssalpha %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-ssfloor")) { /* spectral floor for SS */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-ssfloor check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-ssfloor %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-cvn")) {
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-cvn check_section error");
#endif
        return FALSE;
      }
      jconf->amnow->analysis.para.cvn = 1;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-cvn");
#endif
      continue;
    } else if (strmatch(argv[i],"-nocvn")) {
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-nocvn check_section error");
#endif
        return FALSE;
      }
      jconf->amnow->analysis.para.cvn = 0;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-nocvn");
#endif
      continue;
    } else if (strmatch(argv[i],"-vtln")) { /* VTLN */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-vtln check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-vtln %f %f %f",
           jconf->amnow->analysis.para.vtln_alpha, jconf->amnow->analysis.para.vtln_lower, jconf->amnow->analysis.para.vtln_upper);
#endif
      continue;
    } else if (strmatch(argv[i],"-novtln")) { /* disable VTLN */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-novtln check_section error");
#endif
        return FALSE;
      }
      jconf->amnow->analysis.para.vtln_alpha = 1.0;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-novtln");
#endif
      continue;
    } else if (strmatch(argv[i],"-48")) { /* use 48kHz input and down to 16kHz */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-48 check_section error");
#endif
        return FALSE;
      }
      jconf->input.use_ds48to16 = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-48");
#endif
      continue;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_INFO, "Julius m_options.c", "print version and exit");
#endif
      return FALSE;
    } else if (strmatch(argv[i],"-quiet")) { /* minimum output */
      debug2_flag = verbose_flag = FALSE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-quiet");
#endif
      continue;
    } else if (strmatch(argv[i],"-debug")) { /* debug mode: output huge log */
      debug2_flag = verbose_flag = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-debug");
#endif
      continue;
    } else if (strmatch(argv[i],"-callbackdebug")) { /* output callback debug message */
      callback_debug_flag = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-callbackdebug");
#endif
      continue;
    } else if (strmatch(argv[i],"-progout")) { /* enable progressive output */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-progout check_section error");
#endif
        return FALSE;
      }
      jconf->searchnow->output.progout_flag = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-progout");
#endif
      continue;
    } else if (strmatch(argv[i],"-proginterval")) { /* interval for -progout */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-proginterval check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-proginterval %s", tmparg);
#endif
      continue;
      ...
      jconf->searchnow->output.progout_flag = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-demo");
#endif
      continue;
    } else if (strmatch(argv[i],"-walign")) { /* do forced alignment by word */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-walign check_section error");
#endif
        return FALSE;
      }
      jconf->searchnow->annotate.align_result_word_flag = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-walign");
#endif
      continue;
    } else if (strmatch(argv[i],"-palign")) { /* do forced alignment by phoneme */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-palign check_section error");
#endif
        return FALSE;
      }
      jconf->searchnow->annotate.align_result_phoneme_flag = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-palign");
#endif
      continue;
    } else if (strmatch(argv[i],"-salign")) { /* do forced alignment by state */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-salign check_section error");
#endif
        return FALSE;
      }
      jconf->searchnow->annotate.align_result_state_flag = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-salign");
#endif
      continue;
    } else if (strmatch(argv[i],"-output")) { /* output up to N candidate */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-output check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-output %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-1pass")) { /* do only 1st pass */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-1pass check_section error");
#endif
        return FALSE;
      }
      jconf->searchnow->compute_only_1pass = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-1pass");
#endif
      continue;
    } else if (strmatch(argv[i],"-hlist")) { /* HMM list file */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-hlist check_section error");
#endif
        return FALSE;
      }
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-hlist %s", jconf->amnow->mapfilename);
#endif
      continue;
    } else if (strmatch(argv[i],"-nlr")) { /* word LR n-gram (ARPA) */
      if (!check_section(jconf, argv[i], JCONF_OPT_LM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-nlr check_section error");
#endif
        return FALSE;
      }
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-nlr %s", jconf->lmnow->ngram_filename_lr_arpa);
#endif
      continue;
    } else if (strmatch(argv[i],"-nrl")) { /* word RL n-gram (ARPA) */
      if (!check_section(jconf, argv[i], JCONF_OPT_LM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-nrl check_section error");
#endif
        return FALSE;
      }
      ...
      FREE_MEMORY(jconf->lmnow->ngram_filename);
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-nrl %s", jconf->lmnow->ngram_filename_rl_arpa);
#endif
      continue;
    } else if (strmatch(argv[i],"-lmp")) { /* LM weight and penalty (pass1) */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-lmp check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-lmp %f %f",
          jconf->searchnow->lmp.lm_weight, jconf->searchnow->lmp.lm_penalty);
#endif
      continue;
    } else if (strmatch(argv[i],"-lmp2")) { /* LM weight and penalty (pass2) */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-lmp2 check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-lmp2 %f %f",
          jconf->searchnow->lmp.lm_weight2, jconf->searchnow->lmp.lm_penalty2);
#endif
      continue;
    } else if (strmatch(argv[i],"-transp")) { /* penalty for transparent word */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-transp check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-transp %f", jconf->searchnow->lmp.lm_penalty_trans);
#endif
      continue;
    } else if (strmatch(argv[i],"-gram")) { /* comma-separatedlist of grammar prefix */
      if (!check_section(jconf, argv[i], JCONF_OPT_LM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-gram check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-gram");
#endif
      continue;
    } else if (strmatch(argv[i],"-gramlist")) { /* file of grammar prefix list */
      if (!check_section(jconf, argv[i], JCONF_OPT_LM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-gramlist check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-gramlist %s", tmparg);
#endif
      free(tmparg);
      continue;
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_LM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-userlm check_section error");
#endif
        return FALSE;
      }
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-userlm");
#endif
      continue;
    } else if (strmatch(argv[i],"-nogram")) { /* remove grammar list */
      if (!check_section(jconf, argv[i], JCONF_OPT_LM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-nogram check_section error");
#endif
        return FALSE;
      }
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-nogram");
#endif
      continue;
    } else if (strmatch(argv[i],"-dfa")) { /* DFA filename */
      if (!check_section(jconf, argv[i], JCONF_OPT_LM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-dfa check_section error");
#endif
        return FALSE;
      }
      FREE_MEMORY(jconf->lmnow->dfa_filename);
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-dfa %s", jconf->lmnow->dfa_filename);
#endif
      continue;
    } else if (strmatch(argv[i],"-penalty1")) {	/* word insertion penalty (pass1) */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-penalty1 check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-penalty1 %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-penalty2")) {	/* word insertion penalty (pass2) */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-penalty2 check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-penalty2 %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-spmodel") || strmatch(argv[i], "-sp")) { /* name of short pause word */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-spmodel check_section error");
#endif
        return FALSE;
      }
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-spmodel %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-multipath")) { /* force multipath mode */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-multipath check_section error");
#endif
        return FALSE;
      }
      jconf->amnow->force_multipath = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-multipath");
#endif
      continue;
    } else if (strmatch(argv[i],"-iwsp")) { /* enable inter-word short pause handing (for multipath) */
      if (!check_section(jconf, argv[i], JCONF_OPT_LM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-iwsp check_section error");
#endif
        return FALSE;
      }
      jconf->lmnow->enable_iwsp = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-iwsp");
#endif
      continue;
    } else if (strmatch(argv[i],"-iwsppenalty")) { /* set inter-word short pause transition penalty (for multipath) */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-iwsppenalty check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-iwsppenalty");
#endif
      continue;
    } else if (strmatch(argv[i],"-silhead")) { /* head silence word name */
      if (!check_section(jconf, argv[i], JCONF_OPT_LM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-sihead check_section error");
#endif
        return FALSE;
      }
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-silhead %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-siltail")) { /* tail silence word name */
      if (!check_section(jconf, argv[i], JCONF_OPT_LM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-siltail check_section error");
#endif
        return FALSE;
      }
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-siltail %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-mapunk")) { /* unknown word */
      if (!check_section(jconf, argv[i], JCONF_OPT_LM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-mapunk check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-mapunk %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-iwspword")) { /* add short pause word */
      if (!check_section(jconf, argv[i], JCONF_OPT_LM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-iwspword check_section error");
#endif
        return FALSE;
      }
      jconf->lmnow->enable_iwspword = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-iwspword");
#endif
      continue;
    } else if (strmatch(argv[i],"-iwspentry")) { /* content of the iwspword */
      if (!check_section(jconf, argv[i], JCONF_OPT_LM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-iwspentry check_section error");
#endif
        return FALSE;
      }
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-iwspentry %s", tmparg);
#endif
      continue;
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-iwcache check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-iwcache %f", jconf->searchnow->pass1.iw_cache_rate);
#endif
#else
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_LM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-sepnum check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-sepnum %s", tmparg);
#endif
#else
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-NA check_section error");
#endif
        return FALSE;
      }
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-NA %s", tmparg);
#endif
      continue;
#endif
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-adport check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-adport %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-nostrip")) { /* do not strip zero samples */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-nostrip check_section error");
#endif
        return FALSE;
      }
      jconf->preprocess.strip_zero_sample = FALSE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-nostrip");
#endif
      continue;
    } else if (strmatch(argv[i],"-zmean")) { /* enable DC offset by zero mean */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-zmean check_section error");
#endif
        return FALSE;
      }
      jconf->preprocess.use_zmean = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-zmean");
#endif
      continue;
    } else if (strmatch(argv[i],"-nozmean")) { /* disable DC offset by zero mean */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-nozmean check_section error");
#endif
        return FALSE;
      }
      jconf->preprocess.use_zmean = FALSE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-nozmean");
#endif
      continue;
    } else if (strmatch(argv[i],"-zmeanframe")) { /* enable frame-wise DC offset by zero mean */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-zmeanframe check_section error");
#endif
        return FALSE;
      }
      jconf->amnow->analysis.para.zmeanframe = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-zmeanframe");
#endif
      continue;
    } else if (strmatch(argv[i],"-nozmeanframe")) { /* disable frame-wise DC offset by zero mean */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-nozmeanframe check_section error");
#endif
        return FALSE;
      }
      jconf->amnow->analysis.para.zmeanframe = FALSE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-nozmeanframe");
#endif
      continue;
    } else if (strmatch(argv[i],"-usepower")) { /* use power instead of magnitude in filterbank analysis */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-usepower check_section error");
#endif
        return FALSE;
      }
      jconf->amnow->analysis.para.usepower = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-usepower");
#endif
      continue;
    } else if (strmatch(argv[i],"-nousepower")) { /* use magnitude in fbank analysis (default)  */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-nousepower check_section error");
#endif
        return FALSE;
      }
      jconf->amnow->analysis.para.usepower = FALSE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-nousepower");
#endif
      continue;
    } else if (strmatch(argv[i],"-spsegment")) { /* enable short-pause segmentation */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-sepsegment check_section error");
#endif
        return FALSE;
      }
      jconf->searchnow->successive.enabled = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-sepsegment");
#endif
      continue;
    } else if (strmatch(argv[i],"-spdur")) { /* speech down-trigger duration threshold in frame */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-spdur check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-spdur %s", tmparg);
#endif
      continue;
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-spmargin check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-spmargin %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-spdelay")) { /* speech up-trigger delay frame */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-spdelay check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-spdelay %s", tmparg);
#endif
      continue;
#endif
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-pausemodels check_section error");
#endif
        return FALSE;
      }
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-pausemodels %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-gprune")) { /* select Gaussian pruning method */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-gprune check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-gprune %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-no_ccd")) { /* force triphone handling = OFF */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-no_ccd check_section error");
#endif
        return FALSE;
      }
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-no_ccd");
#endif
      continue;
    } else if (strmatch(argv[i],"-force_ccd")) { /* force triphone handling = ON */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-force_ccd check_section error");
#endif
        return FALSE;
      }
      ...
      jconf->searchnow->force_ccd_handling = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-force_ccd");
#endif
      continue;
    } else if (strmatch(argv[i],"-iwcd1")) { /* select cross-word triphone computation method */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-iwcd1 check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-iwcd1 %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-tmix")) { /* num of mixture to select */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-tmix check_section error");
#endif
        return FALSE;
      }
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-tmix %d", jconf->amnow->mixnum_thres);
#endif
      continue;
    } else if (strmatch(argv[i],"-b2") || strmatch(argv[i],"-bw") || strmatch(argv[i],"-wb")) {	/* word beam width in 2nd pass */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-b2 check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-b2 %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-hgs")) { /* Gaussian selection model file */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-hgs check_section error");
#endif
        return FALSE;
      }
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-hgs %s", jconf->amnow->hmm_gs_filename);
#endif
      continue;
    } else if (strmatch(argv[i],"-booknum")) { /* num of state to select in GS */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-booknum check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-booknum %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-gshmm")) { /* same as "-hgs" */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-gshmm check_section error");
#endif
        return FALSE;
      }
      FREE_MEMORY(jconf->amnow->hmm_gs_filename);
      ...
      jconf->amnow->hmm_gs_filename = filepath(tmparg, cwd);
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-gshmm %s", jconf->amnow->hmm_gs_filename);
#endif
      continue;
    } else if (strmatch(argv[i],"-gsnum")) { /* same as "-booknum" */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-gsnum check_section error");
#endif
        return FALSE;
      } 
      GET_TMPARG;
      jconf->amnow->gs_statenum = atoi(tmparg);
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-gsnum %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-cmnload")) { /* load CMN parameter from file */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-cmnload check_section error");
#endif
        return FALSE;
      }
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-cmnload %s", jconf->amnow->analysis.cmnload_filename);
#endif
      continue;
    } else if (strmatch(argv[i],"-cmnsave")) { /* save CMN parameter to file */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-cmnsave check_section error");
#endif
        return FALSE;
      }
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-cmnsave %s", jconf->amnow->analysis.cmnsave_filename);
#endif
      continue;
    } else if (strmatch(argv[i],"-cmnupdate")) { /* update CMN parameter */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-cmnupdate check_section error");
#endif
        return FALSE;
      }
      jconf->amnow->analysis.cmn_update = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-cmnupdate");
#endif
      continue;
    } else if (strmatch(argv[i],"-cmnnoupdate")) { /* not update CMN parameter */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-cmnnoupdate check_section error");
#endif
        return FALSE;
      }
      jconf->amnow->analysis.cmn_update = FALSE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-cmnnoupdate");
#endif
      continue;
    } else if (strmatch(argv[i],"-cmnmapweight")) { /* CMN weight for MAP */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-cmnmapweight check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-cmnmapweight %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-sscalc")) { /* do spectral subtraction (SS) for raw file input */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-sscalc check_section error");
#endif
        return FALSE;
      }
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-sscalc");
#endif
      continue;
    } else if (strmatch(argv[i],"-sscalclen")) { /* head silence length used to compute SS (in msec) */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-sscalclen check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-sscalclen %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-ssload")) { /* load SS parameter from file */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-ssload check_section error");
#endif
        return FALSE;
      }
      FREE_MEMORY(jconf->amnow->frontend.ssload_filename);
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-ssload %s", jconf->amnow->frontend.ssload_filename);
#endif
      continue;
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-cmalpha check_section error");
#endif
        return FALSE;
      }
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-cmalpha %f %f %f", 
         jconf->searchnow->annotate.cm_alpha_bgn, jconf->searchnow->annotate.cm_alpha_end, jconf->searchnow->annotate.cm_alpha_step);
#endif
#else
      GET_TMPARG;
      jconf->searchnow->annotate.cm_alpha = (LOGPROB)atof(tmparg);
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-cmalpha %s", tmparg);
#endif
#endif
      continue;
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-cmthres check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-cmthres %s", tmparg);
#endif
      continue;
#endif
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-cmthres2 check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-cmthres2 %s", tmparg);
#endif
      continue;
#endif
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-gmm check_section error");
#endif
        return FALSE;
      }
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-gmm %s", jconf->reject.gmm_filename);
#endif
      continue;
    } else if (strmatch(argv[i],"-gmmnum")) { /* num of Gaussian pruning for GMM */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-gmmnum check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-gmmnum %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-gmmreject")) {
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-gmmreject check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-gmmreject %s", tmparg);
#endif
      continue;
#ifdef GMM_VAD
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-gmmmargin check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-gmmmargin %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-gmmup")) { /* uptrigger threshold */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-gmmup check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-gmmup %s", tmparg);
#endif
      continue;
    } else if (strmatch(argv[i],"-gmmdown")) { /* uptrigger threshold */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-gmmdown check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-gmmdown %s", tmparg);
#endif
      continue;
#endif
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-htkconf check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-htkconf %s", tmparg);
#endif
      free(tmparg);
      continue;
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_LM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-wlist check_section error");
#endif
        return FALSE; 
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-wlist %s", tmparg);
#endif
      free(tmparg);
      continue;
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_LM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-wsil check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-wsil %s %s %s",
          jconf->lmnow->wordrecog_head_silence_model_name, jconf->lmnow->wordrecog_tail_silence_model_name, jconf->lmnow->wordrecog_silence_context_name);
#endif
      continue;
#ifdef DETERMINE
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-wed check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-wed %s %s",
          jconf->searchnow->pass1.determine_score_thres, jconf->searchnow->pass1.determine_duration_thres);
#endif
      continue;
#endif
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-inactive check_section error");
#endif
        return FALSE;
      }
      jconf->searchnow->sw.start_inactive = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-inactive");
#endif
      continue;
    } else if (strmatch(argv[i], "-active")) { /* start active (default) */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-active check_section error");
#endif
        return FALSE;
      }
      jconf->searchnow->sw.start_inactive = FALSE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-active");
#endif
      continue;
    } else if (strmatch(argv[i],"-fallback1pass")) { /* use 1st pass result on search failure */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-fallback1pass check_section error");
#endif
        return FALSE;
      }
      jconf->searchnow->sw.fallback_pass1_flag = TRUE;
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-fallback1pass");
#endif
      continue;
#ifdef ENABLE_PLUGIN
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-plugindir %s", tmparg);
#endif
      continue;
#endif
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_LM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-adddict check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-adddict %s", tmparg);
#endif
      free(tmparg);
      continue;
      ...
      if (!check_section(jconf, argv[i], JCONF_OPT_LM)) {
#ifdef ANDROID_DEBUG
        __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-addentry check_section error");
#endif
        return FALSE;
      }
      GET_TMPARG;
...
#ifdef ANDROID_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-addentry %s", tmparg);
#endif
      continue;
    }
    if (argv[i][0] == '-' && strlen(argv[i]) == 2) {
      ...
      switch(argv[i][1]) {
        case 'h':			/* hmmdefs */
          if (!check_section(jconf, argv[i], JCONF_OPT_AM)) {
#ifdef ANDROID_DEBUG
            __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-h check_section error");
#endif
            return FALSE;
          }
...
#ifdef ANDROID_DEBUG
          __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-h %s", jconf->amnow->hmmfilename);
#endif
          break;
        case 'v':			/* dictionary */
          if (!check_section(jconf, argv[i], JCONF_OPT_LM)) {
#ifdef ANDROID_DEBUG
            __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-v check_section error");
#endif
            return FALSE;
          }
          FREE_MEMORY(jconf->lmnow->dictfilename);
...
#ifdef ANDROID_DEBUG
          __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-v %s", jconf->lmnow->dictfilename);
#endif
          break;
        case 'w':			/* word list (isolated word recognition) */
          if (!check_section(jconf, argv[i], JCONF_OPT_LM)) {
#ifdef ANDROID_DEBUG
            __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-w check_section error");
#endif
            return FALSE;
          }
          GET_TMPARG;
...
#ifdef ANDROID_DEBUG
            __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-w failed to read some word list");
#endif
            return FALSE;
          }
#ifdef ANDROID_DEBUG
          __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-w %s", tmparg);
#endif
          break;
        case 'd':			/* binary N-gram */
          ...
          if (!check_section(jconf, argv[i], JCONF_OPT_LM)) {
#ifdef ANDROID_DEBUG
            __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-d check_section error");
#endif
            return FALSE;
          }
          FREE_MEMORY(jconf->lmnow->ngram_filename);
...
#ifdef ANDROID_DEBUG
          __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-d %s", jconf->lmnow->ngram_filename);
#endif
          break;
        case 'b':			/* beam width in 1st pass */
          if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
            __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-b check_section error");
#endif
            return FALSE;
          }
          GET_TMPARG;
...
#ifdef ANDROID_DEBUG
          __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-b %s", tmparg);
#endif
          break;
        case 's':			/* stack size in 2nd pass */
          if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
            __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-s check_section error");
#endif
            return FALSE;
          }
          GET_TMPARG;
...
#ifdef ANDROID_DEBUG
          __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-s %s", tmparg);
#endif
          break;
        case 'n':			/* N-best search */
          if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
            __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-n check_section error");
#endif
            return FALSE;
          }
          GET_TMPARG;
...
#ifdef ANDROID_DEBUG
          __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-n %s", tmparg);
#endif
          break;
        case 'm':			/* upper limit of hypothesis generation */
          if (!check_section(jconf, argv[i], JCONF_OPT_SR)) {
#ifdef ANDROID_DEBUG
            __android_log_print(ANDROID_LOG_ERROR, "Julius m_options.c", "-m check_section error");
#endif
            return FALSE;
          }
          GET_TMPARG;
...
#ifdef ANDROID_DEBUG
          __android_log_print(ANDROID_LOG_DEBUG, "Julius m_options.c", "-m %s", tmparg);
#endif
          break;
        default:
          ...
          //return FALSE;
#ifdef ANDROID_DEBUG
          __android_log_print(ANDROID_LOG_WARN, "Julius m_options.c", "unknown opt");
#endif
          unknown_opt = TRUE;
      }
    } else {			/* error */
